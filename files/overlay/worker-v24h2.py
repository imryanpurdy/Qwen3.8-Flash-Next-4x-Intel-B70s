...
class PleOffloadOutputTarget:
    """Output destination and semaphore for one TP worker."""
...
def _harden_ple_worker() -> None:
    """v17: protect this worker from the OOM killer before any big alloc."""
    import resource

    try:
        resource.setrlimit(resource.RLIMIT_MEMLOCK, (resource.RLIM_INFINITY,) * 2)
    except Exception as exc:
        logger.warning("PLE worker: memlock rlimit raise failed: %s", exc)
    try:
        with open("/proc/self/oom_score_adj", "w") as fh:
            fh.write("-800")
        logger.info("PLE worker: oom_score_adj=-800, memlock rlimit raised (v17).")
    except Exception as exc:
        logger.warning("PLE worker: oom_score_adj set failed: %s", exc)
...
def _init_offload_distributed() -> None:
    ...
    offload_config = VllmConfig(device_config=DeviceConfig(device="cpu"))
...
class PleOffloadWorker:
    ...
    def proc_main(
        ...
        try:
            ...
            # v17: table pins here - after every GPU worker has finished
            # loading, so the touch+mlock pass never races shard loads.
            ready_writer.send(
                {
...
class PleOffloadRunner:
    ...
    def _pin_ple_table(self) -> None:
        """mlock all PLE parameter storage (v16 swap-in-storm fix)."""
        import ctypes
        import resource
        ...
        for name, layer in self._layers.items():
            for pname, param in layer.named_parameters():
                t = param.data
                if t is None or t.device.type != "cpu" or t.numel() == 0:
                    continue
                addr = t.data_ptr()
                nbytes = t.numel() * t.element_size()
                if nbytes == 0:
                    continue
                # Touch pages in order first: mlock prefaults, but an explicit
                # sequential pass gives deterministic, ordered fault-in.
                step = 4096 * 64
                buf = (ctypes.c_char * 1).from_address(addr)
                del buf
                view = memoryview(t.view(torch.uint8).reshape(-1).numpy())
                for off in range(0, nbytes, step):
                    view[off]
                if libc.mlock(ctypes.c_void_p(addr), ctypes.c_size_t(nbytes)) != 0:
                    failed.append((name, pname, nbytes))
                else:
                    locked += nbytes
        if failed:
            logger.error(
                "PLE mlock FAILED for %d tensors (%.1f GB locked); "
                "first failure: %s %s errno=%d",
                len(failed), locked / 2**30, failed[0][0], failed[0][1],
                ctypes.get_errno(),
            )
            raise RuntimeError("PLE table mlock failed - refusing to serve "
                               "with swap-backed table (v16 guard)")
        logger.info(
            "PLE table mlock OK: %.2f GiB locked across %d tensors.",
            locked / 2**30,
            sum(len(list(l.named_parameters())) for l in self._layers.values()),
        )
        try:
            resource.setrlimit(resource.RLIMIT_MEMLOCK, (resource.RLIM_INFINITY,) * 2)
            libc.setpriority(2, 0, -8)  # PRIO_PROCESS, self, -8
            with open("/proc/self/oom_score_adj", "w") as fh:
                fh.write("-800")
            logger.info("PLE worker: memlock rlimit raised, oom_score_adj=-800.")
        except Exception as exc:
            logger.warning("PLE worker post-mlock hardening partial: %s", exc)
    ...
    def _load_weights(self) -> None:
        ...
        def offload_only_iter(
            ...
            for weight_name, tensor in weights:
                ...
                if mapped_name is not None and mapped_name.startswith(offload_prefixes):
                    ...
                    # v18: the ngram table is served from an mmap of the
                    # checkpoint file; never copy it into anon storage.
                    if (".shard_" in weight_name
                            and "ngram_embedding" in weight_name
        ...
        if isinstance(loader, DummyModelLoader):
            ...
            expected_offload_params = {
                ...
                if "ngram_embedding" not in param_name  # v18: mmap-backed
            }
        ...
        # v18: back the ngram table with the checkpoint mmap (zero-copy).
        for layer in offload_layers.values():
            if hasattr(layer, "ngram_embedding"):
                from vllm.v1.ple_offload.ple_mmap_v18 import swap_param_to_mmap
                swap_param_to_mmap(layer)
        ...
        for registration in registrations:
            ...
            for layer_name, gpu_buffer in registration.gpu_output_buffers.items():
                target = PleOffloadOutputTarget(
                    ...
                    copy_stream=(
                        torch.cuda.Stream(device=gpu_buffer.device)

...
#  /opt/venv/bin/vllm /opt/venv/bin/vllm           B70, TP4+EP4, Docker-first (vLLM XPU image + /dev/dri passthrough).
#
...
# early def: preflight disk check uses HF_CACHE_DIR (kit defined it too late)
...
exec_preflight() {
    ...
    if command -v swapon >/dev/null 2>&1; then
        ...
        swap_bytes=$(swapon --show --bytes --noheadings 2>/dev/null | awk '{s+=$3} END{print s+0}' || true)
    fi
...
if command -v python3 >/dev/null 2>&1; then
    python3 - "$ENV_HASH" "$GIT_DESC" "$START_EPOCH" "$watchdog_enabled" <<'PYEOF' > .run/manifest.json
import json, os, sys
env_hash, git_desc, epoch, watchdog_enabled = sys.argv[1:5]
...
DOCKER_RUN+=(--group-add 991)

# --- shared memory for the XPU multi-process stack ---
DOCKER_RUN+=(--shm-size "$SHM_SIZE")
# v16: allow the PLE worker to mlock its 102.4GB table + raise rlimit
DOCKER_RUN+=(--cap-add IPC_LOCK --ulimit memlock=-1:-1)
...
# INT4 lane: --moe-backend triton removed (TRITON WNA16 rejects expert bias; oracle picks XPUExpertsWNA16)                            # Triton MoE = frozen first-load identity (lane3 §1.2)
...
# INT4 overnight lane (2026-09-18): XPU graph capture-replay for decode.
# Evidence: eager CPU-launch saturation measured (4.6-4.9 batched steps/s, worker
# main threads at 100% CPU, wchan=0). a1-a7 quarantine does NOT apply: those died
# in Dynamo/Inductor compile BEFORE capture; FULL_DECODE_ONLY + -O 0 needs no
# piecewise compilation (compilation.py:1485 override only hits PIECEWISE modes).
# v24h (2026-09-19): capture EVERY decode batch size 1..MAX_NUM_SEQS.
...
"${DOCKER_RUN[@]}" "${IMAGE}" /opt/venv/bin/vllm "${VLLM_ARGS[@]}"
